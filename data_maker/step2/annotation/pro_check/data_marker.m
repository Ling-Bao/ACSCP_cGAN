...
% Last Modified by GUIDE v2.5 17-Oct-2015 14:58:01
...
path = f_pa;
path_1 = fullfile(path,'Annotated_data');
path_gtm = fullfile(path,'GT_mat');
...
if(im_need ==0)
    ...
    imname = folder_list(im_ord).name;
    if imname(end-3)=='.'
        gtmat_name = strcat('GT_',imname(1:end-3),'mat');
    end
    if imname(end-2)=='.'
        gtmat_name = strcat('GT_',imname(1:end-2),'mat');
    end 
    correspond_mat = fullfile(path_gtm,gtmat_name);
    load(correspond_mat);
    pop_number = image_info{1}.number;
    current_pt = image_info{1}.number+1;
    label_loc = [];
    pt_location = image_info{1}.location;
    for li = 1:length(image_info{1}.location)
    label_loc =[label_loc, pt_location(li,:)];
    end
    label_image = imread(imglist);
    imshow(label_image);
    hold on;
    for p = 1:length(label_loc)/2
        h_plot(p) = plot(label_loc(2*p-1),label_loc(2*p),'+r');
    end
    mark_crowd();
end
end
...
f_pa(path_length-20:path_length) = [];
path = f_pa;
path_3 = fullfile(path,'Annotated_data');
...
path_4 = fullfile(path,'Checked_data');
movefile(imglist,path_4);
ponit_num = length(label_loc)/2;
...
f_pa(path_length-20:path_length) = [];
path = f_pa;
path_5 = fullfile(path,'Annotated_data');
path_gtm = fullfile(path,'GT_mat');
